...
export const GEOLOCATION_CONSTANTS = {
  ...
  minIdLength: 10,
  minSessionIdLength: 15,
} as const satisfies Record<string, Milliseconds | Meters | Kilometers | number>;
...
export const COPY_DECK = {
  ...
  geoDenied: "Location access denied. You can still search for stations manually.",
  geoTimeout: "Location request timed out. Please try again or search manually.",
  geoUnavailable: "Location services are unavailable. Please search for stations manually.",

  // Error messages
  providerUnavailable: "Service temporarily unavailable. Please try again later.",
  outOfArea: "You're outside the service area. Please search for stations manually.",
...
export type StationCardVariant = (typeof STATION_CARD_VARIANTS)[keyof typeof STATION_CARD_VARIANTS];
export type FareDisplayVariant = (typeof FARE_DISPLAY_VARIANTS)[keyof typeof FARE_DISPLAY_VARIANTS];
...
export type HooksErrorCode = (typeof ERROR_CODES.hooks)[keyof typeof ERROR_CODES.hooks];
