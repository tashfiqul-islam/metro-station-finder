 ...
 */
export function onPerfEntry(entry: {
  name: string;
  value: number;
  id: string;
  rating: string;
  ...
  if (process.env.NODE_ENV === "development") {
    // biome-ignore lint/suspicious/noConsole: Development-only metric logging
    console.info(
      `[Web Vitals] ${emoji} ${entry.name}: ${value}ms (${entry.rating})`
    );
  }
