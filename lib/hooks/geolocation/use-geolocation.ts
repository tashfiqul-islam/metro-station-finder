...
import { GeolocationResultSchema } from "@/lib/config/schemas";
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        const parsed = GeolocationResultSchema.safeParse({
          id: "local",
          coordinates: coords,
          accuracy: Math.max(0, Number(pos.coords.accuracy ?? 0)) as number as unknown as Meters,
...
export function isWithinServiceArea(coords: Coordinates): boolean {
  ...
  return result.success && result.distanceMeters <= DHAKA_SERVICE_AREA.radiusMeters;
}
