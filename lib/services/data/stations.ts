...
import type { Coordinates, Latitude, Longitude, Milliseconds, StationId } from "@/lib/types";
import type {
  MetroLine,
  Station,
  StationAmenities,
...
const MRT6_CONSTANTS = {
  ...
  stationOrder16: 16,
  stationOrder17: 17,
} as const satisfies Record<string, MetroLine | number | StationVersion | string | Milliseconds>;
...
const MRT6_ALIASES: Record<string, readonly string[]> = {
  ...
  agargaon: ["Near Agargaon Market", "Government Zone", "Administrative Center"],
  bijoySarani: ["Near Bijoy Sarani Road", "Shopping Hub", "Shopping Center", "Retail Zone"],
  farmgate: ["Farmgate Intersection", "Major Commercial Hub", "Business District"],
  ...
  dhakaUniversity: ["DU Campus", "TSC", "Student Zone", "Academic District", "University Center"],
  bangladeshSecretariat: [
  ...
  kamalapur: ["Railway Station", "Train Station", "Transport Center", "Intercity Terminal"],
...
const MRT6_METADATA = {
  ...
  bangladeshSecretariat: {
    description: "Station serving Bangladesh Secretariat area (Under Construction)",
    capacity: MRT6_CONSTANTS.capacitySmall,
    platformCount: MRT6_CONSTANTS.platformCount,
    entranceCount: MRT6_CONSTANTS.entranceCountSmall,
    exitCount: MRT6_CONSTANTS.exitCountSmall,
  ...
  kamalapur: {
    description: "Station serving Kamalapur Railway Station area (Under Construction)",
    capacity: MRT6_CONSTANTS.capacityMassive,
    platformCount: MRT6_CONSTANTS.platformCount,
    entranceCount: MRT6_CONSTANTS.entranceCountMassive,
    exitCount: MRT6_CONSTANTS.exitCountMassive,
...
export function getUnderConstructionStations(): readonly Station[] {
  return MRT6_STATIONS.filter((station) => station.status === "under-construction");
}
...
export function getStationByName(name: string): Station | undefined {
  return MRT6_STATIONS.find(
    (station) =>
      ...
      station.aliases.some((alias) => alias.toLowerCase() === name.toLowerCase())
  );
}
...
export function getStationsWithinDistance(
  ...
  return MRT6_STATIONS.filter((station) => {
    ...
    const distance =
      Math.sqrt(latDiff * latDiff + lngDiff * lngDiff) * MRT6_CONSTANTS.metersPerDegree;
 ...
 */
export function getPreviousStation(currentStation: Station): Station | undefined {
  return getStationByOrder(currentStation.order - 1);
}
 ...
 */
export function getStationsBetween(startStation: Station, endStation: Station): readonly Station[] {
  const startOrder = Math.min(startStation.order, endStation.order);
  const endOrder = Math.max(startStation.order, endStation.order);

  return MRT6_STATIONS.filter((station) => station.order > startOrder && station.order < endOrder);
}
...
} as const satisfies Record<string, number | string | StationVersion | Milliseconds>;
