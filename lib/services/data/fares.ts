...
import type { DiscountInfo, DiscountType, Fare, FareBreakdown } from "@/lib/types/fare";
 ...
 */
export function calculateBaseFare(fromStation: string, toStation: string): TakaAmount {
 ...
 */
function calculateDiscountAmount(baseFare: TakaAmount, discountType: DiscountType): TakaAmount {
...
function calculateDistance(fromStation: string, toStation: string): number {
  ...
  return (
    Math.round(
      segments * MRT6_FARE_CONSTANTS.kilometersPerSegment * MRT6_FARE_CONSTANTS.percentageMultiplier
...
function validateFareValues(): boolean {
  for (let i = 0; i < MRT6_FARE_CONSTANTS.matrixSize; i++) {
    ...
    for (let j = 0; j < MRT6_FARE_CONSTANTS.matrixSize; j++) {
      ...
      if (fare < MRT6_FARE_CONSTANTS.minFareAmount || fare > MRT6_FARE_CONSTANTS.maxFareAmount) {
        return false;
      }
...
export const MRT6_FARE_DATA_CONSTANTS = {
  ...
  string,
  string | number | TakaAmount | Milliseconds | boolean | readonly string[] | readonly number[]
>;
