...
import { APP_CONSTANTS, ERROR_CODES } from "@/lib/constants";
import {
  calculateFare as calcFare,
  getFareRules as faresRules,
} from "@/lib/data/fares";
...
export function getFareRules(): ApiResponse<FareRules, never> {
  ...
  const etag = hashStringToEtag(
    `${version}|fareRules|${APP_CONSTANTS.dataVersion}`
  );
  return success(
    rules as unknown as FareRules,
