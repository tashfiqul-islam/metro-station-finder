...
import { APP_CONSTANTS, ERROR_CODES, ID_CONSTANTS, SEARCH_CONSTANTS } from "@/lib/config/constants";
import MRT6_STATIONS, {
  getStationById as getStationByIdFromData,
} from "@/lib/services/data/stations";
import type { ApiResponse, Coordinates, Milliseconds, StationId } from "@/lib/types";
...
type StationErrorCode = (typeof ERROR_CODES.station)[keyof typeof ERROR_CODES.station];
...
export function getStationById(id: StationId): ApiResponse<Station, "STATION_NOT_FOUND"> {
  const start = Date.now() as Milliseconds;
...
export function searchStations(
  ...
  const cacheKey = `${normalized}:${Math.min(limit, SEARCH_CONSTANTS.maxResultsLimit)}`;
  const cached = stationSearchCache.get(cacheKey);
  if (cached !== undefined) {
    const etag = hashStringToEtag(`${version}|search|${cacheKey}`);
    return success(cached.results, (Date.now() - start) as Milliseconds, version, {
      etag,
      dataVersion: APP_CONSTANTS.dataVersion,
    });
  }
  ...
  const requestId = createRequestId(`req_${Date.now().toString(ID_CONSTANTS.base36)}`);
  ...
  if (exact) {
    results.push({
      station: exact,
      searchId: requestId as unknown as import("@/lib/types/station").StationRequestId,
  ...
  const etag = hashStringToEtag(`${version}|search|${cacheKey}|${ranked.length}`);
...
export function findNearestStation(
  ...
  if (!isCoordinates(coordinates)) {
    return failure(ERROR_CODES.station.invalidCoordinates, "Invalid coordinates");
  }

  let best: { station: Station; distance: number } | undefined;
  for (const station of MRT6_STATIONS) {
    const dist = calculateDistance(coordinates, station.coordinates);
    const meters = dist.success ? (dist.distanceMeters as unknown as number) : 0;
  ...
  const result: StationSearchResult = {
    ...
    searchTimestamp: Date.now() as Milliseconds,
    searchQuery: "",
    distance: Math.round(best.distance) as unknown as import("@/lib/types").Meters,
    matchType: "exact",
    relevanceScore: 1,
    isNearest: true,
    rank: 1,
