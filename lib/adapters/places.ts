  ...
  return success(
    {
      ...
      lastResetTime: (status.resetTime - GOOGLE_PLACES_CONSTANTS.rateLimitWindowMs) as Milliseconds,
...
export function searchDestinations(
  ...
  if (q.length < GOOGLE_PLACES_CONSTANTS.minQueryLength) {
    return failure(ERROR_CODES.googlePlaces.invalidQuery, "Query must be at least 3 characters");
  }
  ...
  if (isDailyExceeded()) {
    return failure(ERROR_CODES.googlePlaces.quotaExceeded, "Daily quota exceeded");
  }
  ...
  const response: AutocompleteResponse = {
    ...
    requestId: `req_${Date.now().toString(ID_CONSTANTS.base36)}` as unknown as string,
  ...
  const etag = hashStringToEtag(`${version}|places|${q}|${response.predictions.length}`);
  // Count towards daily usage for accepted request
  dailyUsed += 1;
  return success(response.predictions, (Date.now() - start) as Milliseconds, version, {
    etag,
