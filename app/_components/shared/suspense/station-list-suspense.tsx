...
export function StationListSuspense() {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {featuredStations.slice(0, displayStationCount).map((station: Station) => (
        <div
        ...
        >
          ...
          <p className="text-muted-foreground text-xs">Order: {station.order}</p>
        </div>
      ))}
    </div>
  );
 ...
 */
export function SearchResultsSuspense({ children }: { children: React.ReactNode }) {
  return (
    <Suspense
      fallback={
