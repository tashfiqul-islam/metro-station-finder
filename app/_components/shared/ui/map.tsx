...
import { AlertCircle, Loader2, Maximize2, Navigation, RotateCcw } from "lucide-react";
import { useTheme } from "next-themes";
import { memo, useCallback, useEffect, useMemo, useRef, useState } from "react";
import { LazyMapProviderWrapper } from "@/app/_components/shared/providers/lazy-map-provider";
import { Card, CardContent } from "@/app/_components/shared/ui/card";
import { useMapLoading } from "@/lib/hooks/loading/use-custom-loading";
import { useMapAvailability } from "@/lib/hooks/network/use-map-availability";
...
import { darkModeMapStyle, lightModeMapStyle } from "@/lib/services/map/styles";
...
const MetroMapInner = memo(function MetroMapComponent({
  ...
  const { startMapLoading, completeMapLoading, stopMapLoading } = useMapLoading();
  ...
  // Enhanced station selection handler with optimistic updates
  const handleStationSelect = useCallback(
    ...
    [onStationSelect, updateStationOptimistic, startPageTransition]
  );
  ...
  }, [available, startMapLoading, stopMapLoading]);
  ...
  if (!available) {
    return (
      <Card
        className={cn("rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm", className)}
        style={{ height }}
      >
        <CardContent className="flex h-full flex-col items-center justify-center py-12 text-center">
          <AlertCircle aria-hidden="true" className="mb-4 h-12 w-12 text-muted-foreground" />
          <h3 className="mb-2 font-semibold text-card-foreground text-lg">Map Unavailable</h3>
          ...
          <div className="text-muted-foreground text-xs">Showing station list instead</div>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <MapEffects mapStyle={mapStyle} {...(selectedStation ? { selectedStation } : {})} />

        {/* Map controls overlay */}
        <MapControls center={center} mapContainerRef={mapContainerRef} zoom={zoom} />

        {/* Enhanced station selection handler for future marker integration */}
        {onStationSelect && (
          <button
            aria-label="Click on map to select station"
            className="absolute inset-0 z-10 cursor-pointer border-none bg-transparent p-0"
            onClick={(e) => {
              // This is a placeholder for future station marker clicks
              // When markers are implemented, this will be replaced with actual marker click handlers
              e.preventDefault();

              // For now, we can simulate a station selection for testing
              // This would be replaced with actual station detection from click coordinates
              if (selectedStation) {
